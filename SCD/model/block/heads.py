...
class ResidualUpSample(nn.Module):
    def __init__(self, in_channels):
        ...
        self.top = nn.Sequential(nn.ConvTranspose2d(in_channels, in_channels, 3, stride=2, padding=1, output_padding=1,
                                                    bias=False),
                                 nn.BatchNorm2d(in_channels),
                                 nn.ReLU(True),
                                 nn.Conv2d(in_channels, in_channels // 2, 1, stride=1, padding=0, bias=True)
                                 )
        self.bot = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
                                 nn.Conv2d(in_channels, in_channels // 2, 1, stride=1, padding=0, bias=True)
                                 )
...
class ResidualUpHead(nn.Module):
    def __init__(self, in_channels=128, num_classes=1, dropout_rate=0.15):
        super(ResidualUpHead, self).__init__()

        self.up = nn.Sequential(ResidualUpSample(in_channels),
                                ResidualUpSample(in_channels // 2))
        self.smooth = nn.Sequential(nn.Conv2d(in_channels // 4, in_channels // 4, kernel_size=3, stride=1, padding=1),
                                    nn.ReLU(True),
                                    nn.Dropout2d(dropout_rate))
        self.final = nn.Conv2d(in_channels // 4, num_classes, 1)
